...
Import-Module (Join-Path $PSScriptRoot '..\..\..\scripts\build-addon-collection.psm1') -Force

$projectRoot = (Resolve-Path "$PSScriptRoot\..").Path

# Display build info
$version = Get-ContensiveVersion -DeploymentRoot 'C:\Deployments\aoStatus'
Write-Host ""
Write-Host "========================================"
Write-Host "  aoStatus Build"
...
Invoke-ContensiveBuild `
    -CollectionName    'aoStatus' `
    -CollectionPath    "$projectRoot\collections" `
    -SolutionPath      "$projectRoot\server\aoStatus.sln" `
    -BinPath           "$projectRoot\server\bin\$Configuration\netstandard2.0" `
    -Configuration     $Configuration `
    -DeploymentRoot    'C:\Deployments\aoStatus' `
    -CleanFolders      @(
