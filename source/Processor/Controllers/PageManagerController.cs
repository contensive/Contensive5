    ...
    public class PageManagerController : IDisposable {
        ...
        public static string loadPage(CoreController core, int requestedPageId, DomainModel domain) {
            try {
                ...
                core.docProperties.setProperty("Open Graph Image", (string.IsNullOrEmpty(core.doc.pageController.page.imageFilename.filename)) ? string.Empty : core.webServer.requestProtocol + core.appConfig.domainList.First() + core.appConfig.cdnFileUrl + core.doc.pageController.page.imageFilename.filename);
                return "";
            } catch (Exception ex) {
                logger.Error(ex, $"{core.logCommonMessage}");
                throw;
