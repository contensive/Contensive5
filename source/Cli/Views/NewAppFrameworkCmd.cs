...
namespace Contensive.CLI {
    static class NewAppFrameworkCmd {
        //
        // static logger
        ...
        internal static readonly string helpText = ""
            + Environment.NewLine
            + Environment.NewLine + "--newappframework (-nf) [appName] [primary domain name]"
            + Environment.NewLine + "    Create a new .NET Framework ASPX application (website) on this server."
            + Environment.NewLine + "    Requires DefaultAspxSite.zip in the program files folder."
            + Environment.NewLine + "    If appName is omitted this command acts as a wizard and prompts for all options."
            + Environment.NewLine + "    If primary domain name is omitted, the domain www.appname.com will be used.";
        //
        // ====================================================================================================
        /// <summary>
        /// create a new framework aspx app. If appname is provided, create the app with defaults. if not appname, prompt for defaults
        /// </summary>
        ...
        public static async System.Threading.Tasks.Task executeAsync(string appName, string domainName) {
            try {
                ...
                using (CPClass cp = new CPClass()) {
                    ...
                    //
                    // -- create the new app
                    Console.Write($"\n\nCreate a framework application within the server group [{cp.core.serverConfig.name}]. The application name can contain upper and lower case alpha numeric characters with no spaces or special characters. This name will be used to create and identity resources and should not be changed later.");
                    ...
                    if (!string.IsNullOrEmpty(appName) && cp.core.serverConfig.apps.ContainsKey(appName)) {
                        Console.WriteLine($"The application [{appName}] already exists. To upgrade this app, use --upgrade. To remove this site and create a new site, use --delete first.");
                        return;
                    }
                    ...
                    //
                    // -- admin email address / default from-address, save to site properties after build
                    ...
                    NewAppCmd.setupDirectory(appConfig.localWwwPath);
                    NewAppCmd.setupDirectory(appConfig.localFilesPath);
                    NewAppCmd.setupDirectory(appConfig.localPrivatePath);
                    NewAppCmd.setupDirectory(appConfig.localTempPath);
                    //
                    ...
                    if (!cp.core.serverConfig.isLocalFileSystem) {
                        ...
                        try {
                            ...
                            cp.Log.Error(ex, "NewAppFrameworkCmd, error getting S3 client.");
                        ...
                        try {
                            ...
                            cp.Log.Error(ex, "NewAppFrameworkCmd, error getting S3 policy.");
                        ...
                        try {
                            ...
                            cp.Log.Error(ex, "NewAppFrameworkCmd, error creating S3 policy, continue.");
                            Console.WriteLine($"\nError creating AWS S3 Policy, {ex.Message}");
                            Console.ReadLine();
                        }
                    }
                    ...
                    //
                    logger.Info($"{cp.core.logCommonMessage},Creating database and deploying DefaultAspxSite to IIS site.");
                }
                //
                ...
                using (CPClass cp = new(appName)) {
                    ...
                    cp.core.webServer.verifySite(appName, domainName, cp.core.appConfig.localWwwPath, isFramework: true);
                    //
        ...
        // ====================================================================================================
        /// <summary>
        /// given a domain entered by the user, return the naked domain (without www prefix).
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
